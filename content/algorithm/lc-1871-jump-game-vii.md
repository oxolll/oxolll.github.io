...
lastmod: 2026-05-27T14:39:00+08:00
difficulty: 1896
draft: false
hidden: false
# --- 目錄設定 ---
...
tags: ["leetcode", "string", "dynamic-programming", "binary-search", "prefix-sum", "difference-array", "sliding-window"]
keywords: ["LC1871", "Medium", "1896", "Jump Game VII", "二分搜尋", "區間跳躍", "差分陣列", "前綴和", "滑動窗口"]
description: "LeetCode 第 1871 題：Jump Game VII。難度評分：1896。探討如何利用 lower_bound 二分搜尋驗證區間合法性、進階的差分陣列與前綴和，以及最極致的 O(1) 空間滑動窗口原地標記法，完美解決區間覆蓋問題。"
---
...
> **核心主題**：`String` $\cdot$ `Prefix Sum` $\cdot$ `Binary Search` $\cdot$ `Sliding Window`
...
## 📝 歷次打卡與更新
- [2026-05-27：新增 O(N) 時間與 O(1) 空間的滑動窗口原地標記解法](#解法三-滑動窗口與原地標記-on-時間-o1-空間-極致最佳化)
...
## 💡 解法三：滑動窗口與原地標記 ($\mathcal{O}(N)$ 時間, $\mathcal{O}(1)$ 空間 極致最佳化)

### 🎯 演算法分析
這是一種極度優雅的「原地狀態機」做法，徹底省去了差分陣列或記錄陣列的額外空間開銷。

**核心破局點：動態維護合法起點的窗口**
當我們來到字串的索引 `i` 時，能跳到此處的合法起點必定落在區間 `[i - maxJump, i - minJump]` 中。
我們可以將這個區間視為一個**滑動窗口 (Sliding Window)**，並使用變數 `counter` 來記錄「當前窗口內部共有多少個可達的起點」。
1. **右側進入**：當 `i` 往前走一步時，索引 `i - minJump` 剛好進入窗口，如果它是一個可達點，則 `counter++`。
2. **左側離開**：同時，索引 `i - maxJump - 1` 被擠出窗口，如果它是一個可達點，則 `counter--`。
3. **原地標記**：為了知道歷史上的點是否可達，我們直接將原字串 `s` 中的可達 `'0'` 修改為特殊字元 `'#'`。如此一來，空間複雜度就被完美壓縮到了 $\mathcal{O}(1)$！

### 📊 複雜度分析
- **時間複雜度**: $\mathcal{O}(N)$。僅進行一次線性遍歷，每次迴圈內部皆為 $\mathcal{O}(1)$ 的狀態更新。
- **空間複雜度**: $\mathcal{O}(1)$。直接修改原字串，除了幾個計數變數外不需任何輔助空間。
...
public:
    bool canReach(string s, int minJump, int maxJump) {
        ...
        // counter 記錄當前合法窗口 [i - maxJump, i - minJump] 內的可達點數量
        int counter = 1; 
        
        // 邊界特判：迴圈從 minJump + 1 開始，需提前判斷 minJump 點的狀態
        if(s[minJump] == '0') s[minJump] = '#';

        for(int i = minJump + 1; i < n; ++i) {
            // 1. 窗口右移：新的潛在起點進入合法區間，若該點可達則計數加 1
            if(s[i - minJump] == '#') {
                ++counter;
            }
            
            // 2. 窗口左移：過期的起點離開合法區間，若該點曾經可達則計數減 1
            if(i - maxJump - 1 >= 0 && s[i - maxJump - 1] == '#') {
                --counter;
            }
            
            // 3. 狀態判定：若當前點為 '0' 且窗口內有合法的起點，則標記為可達
            if(s[i] == '0' && counter > 0) {
                s[i] = '#';
            }
        }
        
        // 最後一圈迴圈執行時，若 counter > 0，代表有路徑能抵達終點
        return counter > 0;
    }
};
```
