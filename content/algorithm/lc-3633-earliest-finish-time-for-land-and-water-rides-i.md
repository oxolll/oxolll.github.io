...
lastmod: 2026-06-03T21:40:00+08:00
difficulty: 1342
draft: false
hidden: false
# --- 目錄設定 ---
...
keywords: ["LC3633", "Easy", "1342", "Earliest Finish Time", "貪心演算法", "排程問題", "時間軸重疊"]
description: "LeetCode 第 3633 題：Earliest Finish Time for Land and Water Rides I。探討如何利用貪心策略 (Greedy)，鎖定最早結束的設施作為基準點，以 O(N+M) 線性時間高效找出無縫接軌或最小延遲的最優遊玩排程。"
---

## 📊 題目資訊
> **題目連結**：[LeetCode 3633](https://leetcode.com/problems/earliest-finish-time-for-land-and-water-rides-i/) *(註：題號對應依據實際題庫)* > **難度評分**：<span style="color: #00b8a3; font-weight: bold;">Easy (1342)</span>  
...
如果我們嘗試將所有的陸地設施與所有的水上設施兩兩配對，時間複雜度將會是 $\mathcal{O}(N \times M)$，在資料量大時可能超時。
...
### 📊 複雜度分析
- **時間複雜度**: $\mathcal{O}(N + M)$。其中 $N$ 是陸地設施數量，$M$ 是水上設施數量。我們僅需要對陣列進行幾次獨立的線性掃描，完全避免了雙層迴圈的交叉組合。
