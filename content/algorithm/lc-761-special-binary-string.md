...
### 📊 複雜度分析
- **時間複雜度**: $\mathcal{O}(N^2)$。在最壞情況下，每次遞迴都需要遍歷字串，且字串拼接與排序操作具有較高的時間常數。但考慮到測資限制 $N \le 50$，這種複雜度在執行時間上微乎其微（近乎 0ms）。
- **空間複雜度**: $\mathcal{O}(N^2)$。用於遞迴呼叫堆疊 (Call Stack) 的深度，以及儲存切割後字串 `subss` 的陣列空間。
